...
| Exit animation | `AnimatePresence mode="popLayout"` slide-right + fade (`x: 40, opacity: 0, transition: { duration: 0.2 }`) |
| Clear all confirmation | None — in-memory only, data returns on reload, no confirmation dialog needed |
...
> **Note:** `NotificationsInbox` calls `markAllRead()` on mount, so all items will have `read: true` by the time any dismiss action fires from the inbox UI. The `!target.read` guard is a correctness safeguard for future use cases where dismiss might be triggered before the inbox is opened (e.g., a notification badge dismiss). It does not affect current behavior.
...
Wrap the notification list with `AnimatePresence mode="popLayout"`. The `popLayout` mode causes surrounding items to reflow smoothly while an exiting item animates out, preventing the list from jumping when a mid-list item is dismissed.

```tsx
<AnimatePresence mode="popLayout">
  {notifications.map((n) => (
    <motion.div key={n.id} ... exit={{ x: 40, opacity: 0, transition: { duration: 0.2 } }}>
      ...
    </motion.div>
  ))}
</AnimatePresence>
